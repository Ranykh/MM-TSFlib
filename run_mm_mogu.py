...
import os
import torch
from exp.exp_long_term_forecasting_mm_mogu import Exp_Long_Term_Forecast
...
if __name__ == '__main__':
    ...
    # === MM-MoGU arguments ===
    parser.add_argument('--moe', type=int, default=1, help='use MoE (1) or single model (0)')
    parser.add_argument('--prob_expert', type=int, default=1, help='experts output (pred, variance) for MoGU')
    parser.add_argument('--unc_gating', type=int, default=1, help='use inverse-variance gating (1) or learned gating (0)')
    parser.add_argument('--unc_head_type', type=str, default='mlp', help='uncertainty head architecture: mlp or linear')
    parser.add_argument('--num_experts', type=int, default=2, help='number of experts (homogeneous fallback path)')
    ...
    parser.add_argument('--textual_experts', type=str, nargs='+', default=[],
                        help='list of textual expert model names: BERT, GPT2, GPT2M, GPT2L, GPT2XL, '
                             'LLAMA3 (LLAMA3 needs --huggingface_token); empty for Phase 1')
    ...
    parser.add_argument('--save_outputs', type=int, default=0, help='save pred/true arrays')
    parser.add_argument('--save_visuals', type=int, default=0, help='save prediction plots')
    parser.add_argument('--use_quantile_loss', type=int, default=0, help='quantile loss (unused in MM-MoGU)')
    parser.add_argument('--use_dtw', type=int, default=0, help='compute DTW metric')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            setting = '{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_expand{}_dc{}_fc{}_eb{}_dt{}_{}_{}'.format(
                ...
                args.distil,
                args.des, ii)
            if getattr(args, 'moe', 0):
                setting += '_mmmogu_{}experts_prob{}_uncgate{}'.format(
                    len(args.numeric_experts) + len(args.textual_experts),
                    args.prob_expert, args.unc_gating)

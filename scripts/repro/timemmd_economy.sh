...
#     and the text experts are GPT2, BERT and LLAMA2 -- the same set the gmmts
#     sweep uses, so the two tables line up row for row.
#   --llm_dim defaults to 768 regardless of --llm_model. LLAMA2 needs 4096.
...
# Match the gmmts sweep so the two tables line up: PatchTST is the numeric
# expert everywhere, and the text expert varies.
PAIRS="${PAIRS:-PatchTST:GPT2 PatchTST:BERT PatchTST:LLAMA2}"

# run.py defaults --llm_dim to 768 and never derives it from --llm_model. Passing
# LLAMA2 without correcting this builds the projection at the wrong width.
llm_dim_of () {
  case "$1" in
    LLAMA2|LLAMA3) echo 4096 ;;
    GPT2M)         echo 1024 ;;
    GPT2L)         echo 1280 ;;
    GPT2XL)        echo 1600 ;;
    *)             echo 768  ;;

...
class Model(nn.Module):
    def __init__(self, configs, expert_models):
        super(Model, self).__init__()
        # expert_models: either a list of already-constructed expert instances
        # (heterogeneous MM-MoGU path) or a single class to repeat num_experts
        # times (backward-compatible homogeneous path).
        ...
        if isinstance(expert_models, (list, nn.ModuleList)):
            ...
            self.experts = nn.ModuleList(
                [expert_models(configs).float() for _ in range(self.num_experts)]
            )
